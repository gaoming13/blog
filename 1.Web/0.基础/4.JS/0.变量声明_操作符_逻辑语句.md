...
- 2.位操作符
  - !! 将操作数视为32位元的二进制串(`(1024).toString(2)`)
  - 按位与(AND) `a&b` 规则：`1 && 1 ? 1 : 0`
  - 按位或(OR) `a|b` 规则：`(1 && 0) || (0 && 1) || (1 && 1) ? 1 : 0`
  - 按位异或(XOR) `a^b` 规则：`(1 && 0) || (0 && 1) ? 1 : 0`  例子：`12^5=9`
  - 按位非(NOT) `~a`
    - 规则：按小数点对调所有位(~a = -a - 1)
    - ~5
      - 1. 5 = 00000000 00000000 00000000 00000101
      - 2. 按位取反 = 1111111 1111111 1111111 11111010
      - 3. 由于32位开头是1,所以这是个负数,将二进制转换成负数，需要先反码
