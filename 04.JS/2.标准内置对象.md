...
- 错误类型
  ...
  - RangeError 数值越界时抛出(例:定义数组时设置了不支持的长度,-20或Number.MAX_VALUE)
  - ReferenceError 找不到对象时发生
  - TypeError 变量不是预期的类型或访问不存在的方法
...
- 对象原型属性与方法
  ...
  - 归并方法
    - `Array.prototype.reduce(callback[, thisObject])` 遍历数组, callback(firstVal, secondVal), 每2/3/4...个计算一次, 返回总和
...
### JSON
- JSON(JavaScript Object Notation,JS对象标记)是一种轻量级的数据交换格式，用来存储和表示数据
- JSON.parse()
